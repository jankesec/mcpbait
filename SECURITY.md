...
mcpwn tests agents **you own or have written permission to test**. It is built so that
this is the only thing it can do:

- The server only runs if you add it to an agent configuration by hand. There is no
  path by which mcpwn reaches a third party's agent.
- The beacon binds `127.0.0.1` on a kernel-assigned port. Remote callbacks are not
  implemented.
- Every planted credential is synthetic and carries the literal marker `MCPWN`.
  mcpwn never reads, scans or transmits your real files.
- Reports redact canary values to a prefix, because reports end up in screenshots.

## What mcpwn will not accept
...
mcpwn will happily prove that a shipping product leaks. If that happens:
...
## Reporting a vulnerability in mcpwn itself
